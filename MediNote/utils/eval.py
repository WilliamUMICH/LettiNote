...
NOTE_SPECIFIC_SCORE_TYPES = [] #[f"gpt_{score_type}_score" for score_type in ["clarity", "coherence"]]#, "factuality"]] myadd
...
# os.makedirs(EVAL_DIR, exist_ok=True) # myadd
...
def note_evaluation(model_name, save_path = None ,score_types= NOTE_ALL_SCORE_TYPES): 
    ...
    # path = f'evaluation/{model_name}_evaluation_input.jsonl'
    # '/home/willizhe/wz_stuff/LettiNote/MediNote/data'
    # print('CURRENT CWD: ', os.getcwd())
    path = f'LettiNote/MediNote/data/inference/{model_name}.jsonl' #myadd
    ...
    df = df.rename(columns={'note': 'gold', 'pred_direct-gpt': 'pred'}) # my add
    df = df.rename(columns={'note': 'gold', 'pred_direct': 'pred'}) # my add

...
def extract(
    ...
    if os.path.exists(data_path):
        ...
        notechat_batch = notechat_batch.head(10)
    else:
    ...
    prompts = [
            make_prompts(
                ...
                #note=row['data'] if use_notes else None, myadd
                note=row['note'] if use_notes else None,
    ...
    for i ,(sub_batch_df, nb_tokens) in enumerate(sub_batches):
        ...
        #shutil.copyfile(save_path, f'{save_path[:-6]}{i}.jsonl') #NOTE: when first running code, there is no file to save to. IF you create the missing file, there is nothing to load for line 105. Catch 22

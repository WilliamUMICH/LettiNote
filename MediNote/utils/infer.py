...
Make sure to use all the information from the dialogue into the note, but do not add any new information.""",
    ...
    'mysum-direct': [
        """Given the provided patient-doctor conversation, generate a brief summary of the symptoms.
Make sure to use all the information from the dialogue into the summary, but do not add any new information. Do not change information from what it is written clinical notes.
        """,
        'From this conversation extract the symptoms the patient is reporting:'
    ],
    'mysum-CN': [
        """Given the provided clinical note, generate a brief summary of the symptoms.
Make sure to use all the information from the dialogue into the summary, but do not add any new information. Do not change information from what it is written clinical notes.
        """,
        'From this clinical note extract the symptoms the patient is reporting:'
    ],
    'CN-1shot' : [
        """Given the provided patient-doctor conversation, generate the corresponding clinical note as written by the physician.
Make sure to use all the information from the dialogue into the note, but do not add any new information.""",
...
def format_prompt(model_name, input, mode, instructions, similarCN = None):
    ...
    if 'mistral' in model_name.lower():
        ...
        if similarCN != None:
            prompt = f"<s>[INST] <<SYS>>\n{instructions[0]}\n The following is an example of a clinical note to generate for a different convsersation and different patient: \n\n {similarCN} <</SYS>>\n\n{input}\n\n{instructions[1]} [/INST]"
...
def infer_openai(input_path,
    ...
    prompts = []
    for index, input in tqdm(enumerate(data_df[input_key].tolist()), desc='Building prompts'):

        if shots == 1:
            few_shot_prompt = f"Here is an example of a clinical note.\n\n" + data_df['most_similar_CN'][index]
        else:
            few_shot_prompt = 'Your answer should consist in one or a few paragrpahs of text, not overstructured.'

        prompts.append((f"{instruction}\n\n{few_shot_prompt}\n\n{input}", usr_prompt))

    data_df['messages'] = [build_messages(*prompt) for prompt in tqdm(prompts, desc='creating Datasets')]
    sub_batches = partition(dataframe = data_df, 
                            model = openai_model,
                            max_token_per_partition=max_tokens)
    total_tokens = np.sum([nb_tok for _, nb_tok in tqdm(sub_batches, desc='summing tokens')])
...
def infer_summary(model_name,
    ...
    while not valid and max_tries > 0:
        ...
        # print('TEST 1: ', infer_vllm(client, 'summarizer', prompt))
        # print('TEST 2: ', type(infer_vllm(client, 'summarizer', prompt)))
        partial_answer = starter + infer_vllm(client, 'summarizer', prompt)[0] #myadd list obj
...
def infer(model_name,
    ...
    print('VLM CLIENT CREATED')
    my_i = 0
    for batch in tqdm(data_loader, total=len(data_loader), position=0, desc='running batches'):#leave=True
        my_i += 1
        # logging.info(f'ITERATION: {my_i}')
        # print(f'BATCH :\n{type(batch)}\n')
        # print(batch)
        # print(f'BATCH :\n{batch.keys()}\n')
        # # print(f'data_df :\n{data_df}\n')
        # # print(f'gen_df :\n{gen_df}\n')
        # # print(f'inference_data :\n{inference_data}\n')
        # break
        if mode == 'summarizer':
            prompts = batch[input_key]
            # answers = [infer_summary(model_name, prompts[0], client, template, instructions, verbose=verbose)]
            answers = [infer_summary(model_name, prompts[0], client, template, instructions)] #myadd, verbose doesn't exist
        else: 
            if oneShot:
                similarCN = batch['most_similar_CN']
                prompts = [format_prompt(model_name, input, mode, instructions, similarCN=similarCN) for input in batch[input_key]]
                logging.info(f'PROMPTS: {my_i} \n {prompts}')
            else:
                prompts = [format_prompt(model_name, input, mode, instructions) for input in batch[input_key]]
                print('MY PRINT 2: ', prompts)

            # logging.info(f'ASKING: {my_i}')
            answers = infer_vllm(client, mode, prompts)
            # logging.info(f'DONE ASK: {my_i}')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dialogue',
                        type=str,
                        default=None,
                        help='Dialogue to generate from.')
